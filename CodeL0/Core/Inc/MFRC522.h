...
typedef struct {
	uint8_t		keyByte[6];  //MF_KEY_SIZE  6
} MIFARE_Key;
...
/*typedef struct MFRC522 {
	...
	//static constexpr
	uint8_t FIFO_SIZE[64];		// The FIFO is 64 bytes.
	// Default value for unused pin
	//static constexpr
	//uint8_t UNUSED_PIN[255]; //UINT8_MAX 255

	// MFRC522 registers. Described in chapter 9 of the datasheet.
	// When using SPI all addresses are shifted one bit left in the "SPI address byte" (section 8.1.2.3)
	enum PCD_Register PCD_Register;

	// MFRC522 commands. Described in chapter 10 of the datasheet.
	enum PCD_Command PCD_Command;

	// MFRC522 RxGain[2:0] masks, defines the receiver's signal voltage gain factor (on the PCD).
	// Described in 9.3.3.6 / table 98 of the datasheet at http://www.nxp.com/documents/data_sheet/MFRC522.pdf
	enum PCD_RxGain PCD_RxGain;

	// Commands sent to the PICC.
	enum PICC_Command PICC_Command;

	// MIFARE constants that does not fit anywhere else
	enum MIFARE_Misc MIFARE_Misc;

	// PICC types we can detect. Remember to update PICC_GetTypeName() if you add more.
	// last value set to 0xff, then compiler uses less ram, it seems some optimisations are triggered
	enum PICC_Type PICC_Type;
